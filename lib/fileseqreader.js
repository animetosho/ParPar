...
FileSeqReader.prototype = {
	maxQueuePerFile: 3, // constant: number of queued hash requests per file
	buf: null,
	bufCount: 0,
	maxBufs: 0,
	readSize: 0,
		...
		for(var fileI=0; fileI<this.openFiles.length; fileI++) {
			...
			if(file.hashQueue < this.maxQueuePerFile) { // hash queue not yet full for this file?
				...
				fs.read(file.fd, buffer, 0, readSize[0], null, function(err, bytesRead) {
					...
					if(readSize[1])
						ret.chunks = readSize[1].map(function(offs) {
							return buffer.slice(offs, Math.min(offs + self.reqChunkLen, file.info.size));
						});
					file.hashQueue++;
					file.pos += bytesRead;
					self.cb(null, ret);
