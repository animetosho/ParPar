...
FileSeqReader.prototype = {
	maxQueuePerFile: 3, // constant: number of queued hash requests per file; maybe scale this based on readSize? 3x4MB seems too small in tests (switches on HDD), where 4x4MB is much better, and 5x4MB never switches on HDD
	buf: null,
	bufCount: 0,
	maxBufs: 0,
	readSize: 0,
	...
	readNext: function() {
		...
		// can we fulfill request by reading off open files?
		for(var fileI=0; fileI<this.openFiles.length; fileI++) {
			var file = this.openFiles[fileI];
			if(file.hashQueue < this.maxQueuePerFile) { // hash queue not yet full for this file?
				if(fileI) {
					...
					this.openFiles.unshift(this.openFiles.splice(fileI, 1)[0]);
				}
