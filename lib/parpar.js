...
proto._rfPush = function(numSlices, sliceOffset, critPackets) {
	// TODO: critical packet repetition
	var recvSize = this.par2.packetRecoverySize();
	var packets = Array(numSlices);
	...
	this.recoveryFiles.push({
		name: this.opts.outputBase + module.exports.par2Ext(numSlices, sliceOffset-numSlices, this.opts.recoverySlices + this.opts.recoveryOffset, this.opts.outputAltNamingScheme),
		recoverySlices: numSlices,
		packets: packets.concat(critPackets)
	});
};
...
proto.init = function(cb) {
	...
	module.exports.fileInfo(o.files, function(err, info) {
		...
		this.recoveryFiles = [];
		if(o.outputIndex) this._rfPush(0, 0, critPackets);
		
		// TODO: need to consider offset?!
		if(o.outputSizeScheme == 'pow2') {
			var slices = 1;
			for(var i=0; i<o.recoverySlices; i+=slices, slices*=2)
				this._rfPush(Math.min(slices, o.recoverySlices-i, o.outputFileMaxSlices), i, critPackets);
		} else { // 'equal'
			for(var i=0; i<o.recoverySlices; i+=o.outputFileMaxSlices)
				this._rfPush(Math.min(o.outputFileMaxSlices, o.recoverySlices-i), i, critPackets);
		}
...
proto.start = function() {
	...
	// TODO: generate off this.recoveryFiles ordering, rather than a fixed range
	...
	var outFds = Array(this.recoveryFiles.length);
	var self = this;
	...
	var traverseFileSlices = function(sliceOffset, numSlices, fn, cb) { //refs: self.recoveryFiles, outFds
		...
		async.eachSeries(self.recoveryFiles, function(rf, cb) {
			...
			async.eachSeries(rf.packets, function(pkt, cb) {
				if(pkt.type == 'recovery' && pkt.index >= sliceOffset && pkt.index < endSlice) {
					fn(fd, pkt.index - sliceOffset, filePos, cb)
				} else {
					process.nextTick(cb);
				}
				filePos += pkt.size;
		...
		async.parallel.bind(async, [
			...
			function(cb) {
				...
				async.eachSeries(self.recoveryFiles, function(rf, cb) {
					...
					fs.open(rf.name, 'w', function(err, fd) {
						if(err) return cb(err);
						outFds[idx++] = fd;
			...
			async.eachSeries(self.recoveryFiles, function(rf, cb) {
				...
				async.eachSeries(rf.packets, function(pkt, cb) {
					if(pkt.type == 'recovery') {
						if(pkt.index < rSlices) {
							if(chunker) {
								fs.write(fd, chunker.recoveryData[pkt.index], 0, chunkSize, cPos + module.exports.RECOVERY_HEADER_SIZE, cb);
							} else {
								fs.write(fd, par.recoveryPackets[pkt.index], 0, pkt.size, cPos, cb);
							}
						} else {
							process.nextTick(cb);
						}
						...
						fs.write(fd, critPackets[n], 0, pkt.size, cPos, cb);
					}
					cPos += pkt.size;
				}, cb);
			}, cb);
			...
			async.whilst(function(){return sliceOffset < o.recoverySlices;}, function(cb) {
				...
				async.timesSeries(numChunks-skipChunks, function(n, cb) {
					...
					async.series([
						...
						function(cb) {
							traverseFileSlices(sliceOffset, rSlices, function(fd, sIdx, filePos, cb) {
								var pkt;
								if(chunker) {
									pkt = chunker.recoveryData[sIdx];
									filePos += chunkOffset + module.exports.RECOVERY_HEADER_SIZE;
								} else
									pkt = par.recoveryPackets[sIdx];
					...
					// write chunk headers
					traverseFileSlices(sliceOffset - rSlices, rSlices, function(fd, sIdx, filePos, cb) {
						var data = chunker.getHeader(sIdx);
