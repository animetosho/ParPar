...
proto.init = function(cb) {
	...
	module.exports.fileInfo(this.opts.files, function(err, info) {
		...
		switch(this.opts.displayNameFormat) {
...
proto.start = function() {
	...
	if(o.outputSizeScheme == 'pow2') {
		...
		for(var i=0; i<o.recoverySlices; i+=slices, slices*=2) {
			fileSlices.push(Math.min(slices, o.recoverySlices-i, o.outputFileMaxSlices));
		}
	} else { // 'equal'
	...
	async.series([
		async.parallel.bind(async, [
			...
			function(cb) {
				...
				async.eachSeries(fileSlices, function(numSlices, cb) {
					...
					fs.open(o.outputBase + module.exports.par2Ext(numSlices, sliceOffset-numSlices, o.recoverySlices + o.recoveryOffset, o.outputAltNamingScheme), 'w', function(err, fd) {
						if(err) return cb(err);
						outFds[idx++] = fd;
						...
						if(chunker || sliceOffset > rSlices) {
							...
							var allocSize = par.packetRecoverySize() * numSlices;
		...
		function(cb) {
			...
			async.eachSeries(fileSlices, function(numSlices, cb) {
				var fd = outFds[fdI++];
				var recSize = par.packetRecoverySize() * numSlices;
				(function(cb) {
