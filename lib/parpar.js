...
proto.start = function() {
	...
	var recoveryPacketSize = par.packetRecoverySize();
	...
	async.series([
				...
				async.eachSeries(self.recoveryFiles, function(rf, cb) {
					...
					fs.open(rf.name, 'w', function(err, fd) {
						...
						if(chunker || sliceOffset > rSlices) {
							...
							var allocSize = par.packetRecoverySize() * rf.recoverySlices;
		...
		function(cb) {
			...
			async.eachSeries(self.recoveryFiles, function(rf, cb) {
				var fd = outFds[fdI++];
				var recSize = par.packetRecoverySize() * rf.recoverySlices;
				(function(cb) {
					var cPos = chunker ? module.exports.RECOVERY_HEADER_SIZE : 0;
					async.timesSeries(Math.min(rf.recoverySlices, Math.max(0, rSlices - recPktI)), function(slice, cb) {
						var pkt;
						if(chunker) {
