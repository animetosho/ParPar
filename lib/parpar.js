	...
	async.eachSeries(this.recoveryFiles, function(rf, cb) {
		...
		async.eachSeries(rf.packets, function(pkt, cb) {
			if(pkt.type == 'recovery' && pkt.index >= sliceOffset && pkt.index < endSlice) {
				fn(rf.fd, pkt.index - sliceOffset, filePos, cb)
			} else {
				process.nextTick(cb);
			}
			filePos += pkt.size;
...
proto.start = function() {
	...
	async.series([
				...
				async.eachSeries(rf.packets, function(pkt, cb) {
					if(pkt.type == 'recovery') {
						if(pkt.index < rSlices) {
							if(chunker) {
								fs.write(rf.fd, chunker.recoveryData[pkt.index], 0, chunkSize, cPos + module.exports.RECOVERY_HEADER_SIZE, cb);
							} else {
								fs.write(rf.fd, par.recoveryPackets[pkt.index], 0, pkt.size, cPos, cb);
							}
						} else {
							process.nextTick(cb);
						}
			...
			async.whilst(function(){return sliceOffset < o.recoverySlices;}, function(cb) {
				...
				async.timesSeries(numChunks-skipChunks, function(n, cb) {
					...
					async.series([
						...
						function(cb) {
							self._traverseRecoveryPacketRange(sliceOffset, rSlices, function(fd, sIdx, filePos, cb) {
								var pkt;
								if(chunker) {
					...
					// write chunk headers
					self._traverseRecoveryPacketRange(sliceOffset - rSlices, rSlices, function(fd, sIdx, filePos, cb) {
		...
		// close output files
		async.eachSeries.bind(async, self.recoveryFiles.map(function(rf) {
			return rf.fd;
		}), fs.close.bind(fs))
	], function(err) {
		// TODO: cleanup on err
