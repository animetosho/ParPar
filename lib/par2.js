...
if(gf.AlignedBuffer) {
	AlignedBuffer = function(len) {
		var buf = gf.AlignedBuffer(len);
		buf.offset = 0; // bugfix for some versions of node which assumes this exists
		return buf;
	};
} else {
	...
	bufferedFinish: function(cb, clear, md5) {
		if(!this.bgProcessInputs) {
			...
			if(this.qInputReady && this.bufferedInputPos) {
				...
				gf.finish(this.recoveryData, md5); // TODO: this could be optimized
				this._processStarted = false;
				this.bufferedClear(!clear);
				process.nextTick(cb);
			}
...
var GF_METHODS = [
	'' /*default*/, 'lh_lookup', 'xor', 'shuffle', 'affine'
];
...
module.exports = {
	...
	setMethod: function(method, wordBits, sliceSize) {
		...
		gfMethod = gf.set_method(meth, wordBits, sliceSize);
	},
	getMethod: function() {
		return {
