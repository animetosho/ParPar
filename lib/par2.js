...
var GFWrapper = {
	...
	gf_init: function(size) {
		this.close();
		this.gf = new binding.GfProc(size, this._gfOpts.method, this._gfOpts.batchSize, this._gfOpts.stagingCount);
...
function PAR2(files, sliceSize, opts) {
	...
	if(opts.method)
		opts.method = getGFMethodNum(opts.method);
	else
...
PAR2File.prototype = {
	...
	processHash: function(data, cb) {
		...
		while(this._slicePartPos + data.length >= this.par2.sliceSize) {
			if(this.sliceHashPos >= this.numSlices)
				throw new Error('Too many slices given');
			
			var dataPart = data.slice(0, this.par2.sliceSize - this._slicePartPos);
			data = data.slice(dataPart.length);
			
			if(this.pktCheck)
				tasks.push(this._md5ctx.updateGet.bind(this._md5ctx, dataPart, this._pktCheckHashSlice(), 0));
			this.sliceHashPos++;
			this._slicePartPos = 0;
		}
		if(data.length) { // remaining data
			this._slicePartPos += data.length;
			if(this.pktCheck)
				tasks.push(this._md5ctx.update.bind(this._md5ctx, data));
		}
		
		// handle end condition
		var getLastBlock;
		if(this.sliceHashPos == this.numSlices-1) {
			var rem = this.size % this.par2.sliceSize;
			if(rem > 0) {
				if(this._slicePartPos > rem) {
					throw new Error('Invalid data length for last slice');
				} else if(this._slicePartPos == rem) {
					// reached end - flush
					if(this.pktCheck)
						getLastBlock = [this._pktCheckHashSlice(), this.par2.sliceSize - this._slicePartPos];
					this.sliceHashPos++;
					this._slicePartPos = 0;
				}
			}
		...
		if(this.sliceHashPos >= this.numSlices) {
			...
			if(this._slicePartPos != 0)
				throw new Error('Invalid data length for last slice');
			getFinalHash = !this.md5;
		}
		
		var self = this;
		async.parallel(tasks, function() {
			if(getLastBlock)
				self._md5ctx.getBlock.apply(self._md5ctx, getLastBlock);
			if(getFinalHash) {
...
var getGFMethodNum = function(method) {
	if(typeof method == 'number') {
		if(!(method in GF_METHODS))
...
module.exports = {
	...
	gf_info: function(method) {
		return binding.gf_info(getGFMethodNum(method));
	},
