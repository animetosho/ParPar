	...
	_getRecoveryPacketMD5: function(idx, cb) {
		...
		(function(cb) {
			...
			if(self.recDataHashers && self.recDataHashers[hasherIdx]) { // if not finished
				...
				var numBufs = Math.min(self._gfOpts.hashBatchSize, self.recoverySlices.length - hashBaseIdx);
				for(var i=0; i<numBufs; i++) {
					self.recDataHashers[hasherIdx].get(self.recDataMD5.slice(i*16), i);
				}
				// deallocate hasher
...
PAR2File.prototype = {
	...
	_pktCheckHashSlice: function() {
		return this.pktCheck.slice(64 + 16 + 20*this.slicePos);
	},
	...
	process: function(data, cb) {
		...
		async.parallel([
			function(cb) {
				...
				self._md5ctx.updateGet(data, self._pktCheckHashSlice(), self.par2.sliceSize - data.length, function() {
					if(lastPiece) {
	...
	processHashEnd: function() {
		...
		if(this.slicePos < this.numSlices) {
			...
			if(this.pktCheck)
				this._md5ctx.getBlock(this._pktCheckHashSlice(), this.par2.sliceSize - this._slicePartPos);
			this.slicePos++;
		} else {
			if(this.slicePos != this.numSlices)
				throw new Error('Not at last slice');
		...
		if(!this.md5) {
			this.md5 = allocBuffer(16);
			this._md5ctx.end(this.md5);
			this._md5ctx = null;
		}
	},
	processHash: function(data, cb) {
		...
		async.whilst(function() { return self._slicePartPos + data.length >= self.par2.sliceSize; }, function(cb) {
			...
			if(self.pktCheck) {
				self._md5ctx.updateGet(dataPart, self._pktCheckHashSlice(), 0, function() {
					self.slicePos++;
					cb();
				});
			} else {
