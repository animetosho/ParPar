...
if(!AlignedBuffer) {
	...
	AlignedBuffer = function(len) {
		var buf = new Buffer(len + gfMethod.alignment-1);
...
function PAR2(files, sliceSize) {
	...
	this.files = files.map(function(file) {
		...
		return BufferCompare(Buffer(a.id).reverse(), Buffer(b.id).reverse());
	});
...
PAR2File.prototype = {
	...
	getPacketChecksums: function(keep) {
		if(!this.numSlices) return new Buffer(0);
...
module.exports = {
	...
	setMethod: function(method, chunkSize) {
		...
		gfMethod = gf.set_method(meth, chunkSize);
	},
	getMethod: function() {
		return {
