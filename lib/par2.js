...
PAR2File.prototype = {
	...
	_pktCheckHashSlice: function() {
		return this.pktCheck.slice(64 + 16 + 20*this.sliceHashPos);
	},
	...
	processData: function(data, cb) {
		...
		var lastPiece = this.sliceDataPos == this.numSlices-1;
		...
		this.par2.processSlice(data, this.sliceOffset + this.sliceDataPos, cb);
		this.sliceDataPos++;
	},
	...
	processHash: function(data, cb) {
		...
		async.whilst(function() { return self._slicePartPos + data.length >= self.par2.sliceSize; }, function(cb) {
			...
			if(self.sliceHashPos >= self.numSlices) return cb(new Error('Too many slices given'));
			...
			(function(cb) {
				...
				if(self.sliceHashPos == self.numSlices-1) {
					var rem = self.size % self.par2.sliceSize;
					if(rem > 0) {
						if(self._slicePartPos > rem) {
							throw new Error('Invalid data length for last slice');
						} else if(self._slicePartPos == rem) {
							// reached end - flush
							if(self.pktCheck)
								self._md5ctx.getBlock(self._pktCheckHashSlice(), self.par2.sliceSize - self._slicePartPos);
							self.sliceHashPos++;
							self._slicePartPos = 0;
						}
					}
				...
				if(self.sliceHashPos >= self.numSlices) {
					if(self.sliceHashPos != self.numSlices)
						throw new Error('Hashed past last slice');
					if(self._slicePartPos != 0)
						throw new Error('Invalid data length for last slice');
