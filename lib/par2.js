...
var GFWrapper = {
	...
	gf_init: function(size) {
		this.close();
		this.gf = new binding.GfProc(size, this._gfOpts.method, this._gfOpts.batchSize);
		if(this._gfOpts.threads)
	...
	bufferedProcess: function(dataSlice, sliceNum, len, cb) {
		...
		if(!this.addQueue) {
			this.addQueue = [];
			this.gf.setProgressCb(function(numInputs, firstInput) {
	...
	finish: function(files, cb) {
		...
		if(files) {
			files.forEach(function(file) {
				...
				// TODO: is the following necessary? probably should mandate that first pass always generates MD5
				if(!file.md5) // shouldn't happen, but if full data wasn't given for first pass, try again?
					file._md5ctx = new binding.HasherInput();
			});
		}
	...
	_pullRecData: function() {
		...
		this.recData = Array(Math.min(this.recoverySlices.length, hashBatchSize*2));
		...
		this.recDataMD5 = Array(Math.min(this.recoverySlices.length, hashBatchSize));
	...
	_getRecoveryPacketMD5: function(idx, cb) {
		...
		(function(cb) {
			...
			if(self.recDataHashers && self.recDataHashers[hasherIdx]) { // if not finished
				// finish hashing
				self.recDataHashers[hasherIdx].end();
				// pull out MD5s
				var hashBaseIdx = hasherIdx*self._gfOpts.hashBatchSize;
				var numBufs = Math.min(self._gfOpts.hashBatchSize, self.recoverySlices.length - hashBaseIdx);
				for(var i=0; i<numBufs; i++) {
					self.recDataMD5[i] = self.recDataHashers[hasherIdx].get(i);
				}
				// deallocate hasher
			...
			cb(self.recDataMD5[idx % self._gfOpts.hashBatchSize]);
		});
	},
...
function PAR2(files, sliceSize, opts) {
	...
	if(opts.method) {
		var meth = GF_METHODS.indexOf(opts.method);
		if(meth < 0) throw new Error('Unknown method "' + opts.method + '"');
		opts.method = meth;
	} else opts.method = undefined;
...
function PAR2File(par2, file) {
	...
	this._md5ctx = new binding.HasherInput();
...
PAR2File.prototype = {
	...
	_writeBlockHash: function(zeroPad) {
		this._md5ctx.getBlock(zeroPad).copy(this.pktCheck, 64 + 16 + 20*this.slicePos);
	},
	...
	process: function(data, cb) {
		...
		if((this.pktCheck || !this.md5) && !(this.pktCheck && !this.md5))
			throw new Error('Cannot generate check packet or MD5 without the other');
		
		var self = this;
		async.parallel([
			function(cb) {
				if(!self.pktCheck) return cb();
				self._md5ctx.update(data, function() {
					self._writeBlockHash(self.par2.sliceSize - data.length);
					if(lastPiece) {
						self.md5 = self._md5ctx.end();
						self._md5ctx = null;
					}
					cb();
				});
			},
			this.par2.processSlice.bind(this.par2, data, this.sliceOffset + this.slicePos)
		], function() {
			self.slicePos++;
			cb();
		});
	...
	processHashEnd: function() {
		...
		// zero fill & write last slice
		// (since we always process full blocks, only do this if we have a partial block)
		if(this.slicePos < this.numSlices) {
			if(this.slicePos != this.numSlices-1)
				throw new Error('Not at last slice');
			if(rem != this._slicePartPos)
				throw new Error('Invalid data length for last slice');
			
			if(this.pktCheck)
				this._writeBlockHash(this.par2.sliceSize - this._slicePartPos);
			this.slicePos++;
		} else {
			if(this.slicePos != this.numSlices)
				throw new Error('Not at last slice');
		...
		if(!this.md5) {
			this.md5 = this._md5ctx.end();
			this._md5ctx = null;
		}
	},
	processHash: function(data, cb) {
		var self = this;
		async.whilst(function() { return self._slicePartPos + data.length >= self.par2.sliceSize; }, function(cb) {
			var dataPart = data.slice(0, self.par2.sliceSize - self._slicePartPos);
			
			if(self.slicePos >= self.numSlices) return cb(new Error('Too many slices given'));
			
			self._slicePartPos = 0;
			data = data.slice(dataPart.length);
			
			if(self.pktCheck) {
				self._md5ctx.update(dataPart, function() {
					self._writeBlockHash(0);
					self.slicePos++;
					cb();
				});
			} else {
			...
			// feed thru remainder
			if(data.length) {
				self._slicePartPos += data.length;
				if(self.pktCheck) {
					self._md5ctx.update(data, cb);
					return;
				}
			}
			cb();
