...
// for some reason, creating Buffers natively in node4 causes stability issues; TODO: investigate why
if(!process.version.match(/^v[012]\./)) {
	// for emulating an AlignedBuffer with native node Buffers
...
PAR2.prototype = {
	...
	setRecoverySlices: function(slices) {
		...
		if(this.recoveryData && this.recoveryData.length == this.recoverySlices.length) {
			// if the number of slices hasn't changed, we don't need to reallocate - just update numbers
			...
			for(var i in this.recoverySlices) {
				this.recoveryPackets[i] = AlignedBuffer(size).slice(headerSize - 68); // offset for alignment purposes
				this.recoveryPackets[i].writeUInt32LE(this.recoverySlices[i], 64);
...
PAR2Chunked.prototype = {
	...
	setChunkSize: function(size) {
		...
		// TODO: avoid reallocation if possible
		this.chunkSize = size;
		for(var i in this.recoverySlices)
			this.recoveryData[i] = size ? AlignedBuffer(size) : null;
