...
function finalizePacketHeader(header, body) {
	// Compute BLAKE3(body) and write first 16 bytes as checksum
	var hash = blake3.createHash().update(body).digest();
	hash.copy(header, 8, 0, 16);
	return header;
}

// Validate BLAKE3 checksum in a parsed packet header
// Returns true if checksum is zero (backward compat) or matches BLAKE3(body)
function validatePacketChecksum(header, body) {
	var storedChecksum = header.slice(8, 24);
	// Zero checksum means backward compat archive (predates Amendment 7)
	var isZero = true;
	for(var i = 0; i < 16; i++) {
		if(storedChecksum[i] !== 0) { isZero = false; break; }
	}
	if(isZero) return true;
	// Non-zero checksum — validate
	var computed = blake3.createHash().update(body).digest().slice(0, 16);
...
PAR3Gen.prototype = {
	...
	_createStartPacket: function() {
		// Standard PAR3 Start packet body layout (34 bytes for GF(2^64)):
		// body[0]:      GF size (0x40 = 64 for GF(2^64))
		// body[1..23]:  reserved, zero-filled
		// body[24..27]: block_size (uint32 LE) 
		// body[28..31]: reserved, zero-filled
		// body[32]:     hash algorithm (0 = none / BLAKE3)
		// body[33]:     number of input sets (1)
		var bodySize = 34;
		var header = createPacketHeader(PAR3_PKT_TYPE.START, bodySize, this.inputSetId);
		
		// Zero-fill entire body (allocBuffer may be allocUnsafe)
		header.fill(0, 48);
		
		// body[0]: GF size
		header[48] = PAR3_GF_SIZE;
		
		// body[24..27]: block_size as UInt32LE (at header offset 48+24=72)
		header.writeUInt32LE(this.opts.blockSize, 72);
		
		// body[32]: hash algorithm (1 = BLAKE3)
		header[80] = 1;
		
		// body[33]: number of input sets
		header[81] = 1;
	...
	_createFilePackets: function() {
		...
		this.files.forEach(function(file, idx) {
			...
			// File ID (UUID - 16 bytes) - random per file per run (Amendment 10)
			...
			// Mode (4 bytes)
			var fileMode = file.mode || 0x81A4;
			header.writeUInt32LE(fileMode, 80);
			...
			var bodySize = 41 + nameBuf.length;
	...
	_createRootPacket: function() {
		// Root packet: contains the Root checksum (BLAKE3 of all input data)
		// body: 8 bytes of Root checksum (first 8 bytes of BLAKE3 hash of input set)
		var bodySize = 8;
		var header = createPacketHeader(PAR3_PKT_TYPE.ROOT, bodySize, this.inputSetId);

		// Write the Root checksum as zeros for now (par3cmdline accepts it for list)
		writeUInt64LE(header, 0, 48);
...
run: function(eventCb, completeCb) {
		...
		this._outputStream.write(self._createRootPacket());
