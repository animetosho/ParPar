...
run: function(eventCb, completeCb) {
		...
		function ensureAccumulator() {
			...
			// Must zero-init (NOT allocBuffer=allocUnsafe): Cauchy RHS leaves
			// unwritten bytes in the accumulator that corrupt recovery output.
				...
				function readBlockFull() {
					while(true) {
						...
						if(remaining <= 0) {
							...
							// A3: pool reset after each file completes so the
							// next file starts at offset 0 (matches the
							// "after each file completes" spec).
							_resetInputBufferPool();
							processFileFull(idx + 1);
							return;
						}
						...
						var buf = acquireInputBuffer(toRead);
