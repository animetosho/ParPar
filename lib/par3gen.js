...
// GF64 binding - never load to avoid segfault
...
function invert64(val) {
	...
	while(u !== 1n && u !== 0n) {
		while((u & 1n) === 0n) {
			u >>= 1n;
			if((x1 & 1n) !== 0n) {
				x1 = ((x1 + v) >> 1n) & 0xFFFFFFFFFFFFFFFFn;
			} else {
				x1 >>= 1n;
			}
		}
	...
	_generateRecoveryBlocks: function(inputBlocks, eventCb, cb) {
		...
		for(var recIdx = 0; recIdx < numRecovery; recIdx++) {
			...
			for(var inpIdx = 0; inpIdx < numInput; inpIdx++) {
				...
				coeffs.push(invert64(denom));
			}
			
			var recoveryData = Buffer.alloc(blockSize);
			
			for(var elemIdx = 0; elemIdx < blockSize / 8; elemIdx++) {
				var acc = 0n;
				
				for(var inpIdx = 0; inpIdx < numInput; inpIdx++) {
					var val = inputBlocks[inpIdx].readBigUInt64LE(elemIdx * 8);
					acc ^= gf64_mul(val, coeffs[inpIdx]);
				}
				
				recoveryData.writeBigUInt64LE(acc & 0xFFFFFFFFFFFFFFFFn, elemIdx * 8);
			}
			
			recoveryBlocks.push({
				blockIndex: numInput + recIdx,
				data: recoveryData
			});
		}
...
function par3_repair(par3File, outputDir, opts, cb) {
	...
	var packetCallback = function(type, body, body_length) {
		...
		switch(type) {
			case PAR3_PKT_TYPE.START: {
				console.log('DEBUG: START case - body[0]=', body[0], 'body[32]=', body[32]);
				...
				console.log('DEBUG: START - blockSize set to', repairState.blockSize);
				break;
			}
			case PAR3_PKT_TYPE.MATRIX: {
				repairState.matrix = {
			...
			case PAR3_PKT_TYPE.DATA: {
				console.log('DEBUG: DATA case - body.length=', body.length, 'blockSize=', repairState.blockSize);
				var dataBlock = {
					block_index: readUInt64LE(body, offset),
					data: body.slice(offset + 8, offset + 8 + repairState.blockSize)
				};
				console.log('DEBUG: dataBlock pushed - block_index=', dataBlock.block_index, 'data.length=', dataBlock.data.length);
	...
	fs.readFile(par3File, function(err, data) {
		...
		par3_parse_buffer(data, packetCallback, function(err2) {
			console.log('DEBUG: parse complete, err2=', err2);
			if(err2) return cb(new Error('Failed to parse PAR3 file: ' + err2));

			console.log('DEBUG: startPacket=', repairState.startPacket ? 'yes' : 'no');
			console.log('DEBUG: matrix=', repairState.matrix ? 'yes' : 'no');
			console.log('DEBUG: dataBlocks=', repairState.dataBlocks.length);
			console.log('DEBUG: recoveryData=', repairState.recoveryData.length);
			...
			console.log('DEBUG: blockSize=' + repairState.blockSize);
			...
			var availableMap = {};
			console.log('DEBUG: dataBlocks count=' + repairState.dataBlocks.length);
			...
			console.log('DEBUG: totalBlocks=' + totalBlocks);
...
function par3_parse_buffer(buffer, callback, done) {
	...
	function readPacket() {
		pktCount++;
		if(pktCount > 20) {
			console.log('DEBUG: too many packets, stopping');
			return done();
		}
		if(offset + 48 > buffer.length) {
			console.log('DEBUG: offset+48 > length, calling done');
			return done();
		}
		...
		console.log('DEBUG: pktLen =', pktLen, 'bodyLen =', bodyLen);
		...
		console.log('DEBUG: typeStr=', typeStr);
		...
		console.log('DEBUG: mapped type=', type);

		try {
			console.log('DEBUG: calling callback for type', type);
			callback(type, body, bodyLen);
			console.log('DEBUG: callback returned');
		} catch(e) {
			console.log('DEBUG: callback threw:', e.message);
			return done(e);
		}

		offset += pktLen;
		console.log('DEBUG: offset now', offset, ', buffer.length', buffer.length);
		if(offset < buffer.length) {
			return readPacket();
		}
		console.log('DEBUG: end of packets, calling done');
		console.log('DEBUG: done function type:', typeof done);
		done();
		console.log('DEBUG: done() returned');
	}

	readPacket();
	console.log('DEBUG: readPacket returned');
}
