...
function finalizePacketHeader(header, body) {
	...
	var hash = blake3.createHash().update(afterChecksum).digest();
...
function validatePacketChecksum(header, body) {
	...
	var afterChecksum = header.slice(24);
	var computed = blake3.createHash().update(afterChecksum).digest().slice(0, 16);
...
PAR3Gen.prototype = {
	...
	_createFilePackets: function() {
		...
		this.files.forEach(function(file, idx) {
			...
			var bodySize = 4 + nameBuf.length + 16 + 4 + 8 + 4;
			...
			header.writeUInt32LE(file.size, 52 + nameBuf.length + 16);
			writeUInt64LE(header, 0, 52 + nameBuf.length + 20);
			header.writeUInt32LE(file.mode || 0x81A4, 52 + nameBuf.length + 28);
