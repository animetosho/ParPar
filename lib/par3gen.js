...
function par3_verify(par3File, cb) {
	...
	var packetCallback = function(type, body, body_length, header) {
		...
		switch(type) {
			...
			case PAR3_PKT_TYPE.MATRIX: {
				...
				if(body.length >= 32) {
					verifyState.matrix.recovery_count = readUInt64LE(body, offset + 24);
					verifyState.matrix.recovery_count_present = true;
				} else {
					verifyState.matrix.recovery_count = 0;
					verifyState.matrix.recovery_count_present = false;
				}
	...
	par3_parse_stream(par3File, packetCallback, function(err) {
		...
		if(verifyState.recoveryCount === 0 && verifyState.matrix.recovery_count_present) {
	...
	var packetCallback = function(type, body, body_length) {
		...
		switch(type) {
			...
			case PAR3_PKT_TYPE.MATRIX: {
				...
				if(body.length >= 32) {
					repairState.matrix.recovery_count = readUInt64LE(body, offset + 24);
				} else {
					repairState.matrix.recovery_count = 0;
				}
				if(body.length >= 40) {
					repairState.matrix.rows = body.readUInt32LE(offset + 32);
					repairState.matrix.cols = body.readUInt32LE(offset + 36);
				} else {
					repairState.matrix.rows = 0;
					repairState.matrix.cols = 0;
				}
