...
PAR3Gen.prototype = {
	...
	_generateRecoveryBlocks: function(inputBlocks, eventCb, cb) {
		...
		// The addon's Gf64Encoder_create factory currently returns a raw NAPI
		// External (pointer wrapper) which does not expose mul_arr — only
		// instances of the Gf64Encoder NAPI class do. Upgrade at the call
		// boundary so the refactored body can use the native binding.
		...
		for(var recIdx = 0; recIdx < numRecovery; recIdx++) {
			...
			// Pre-allocate ONCE per recovery block (NOT per input block).
			// For -r 16 on 1MB blocks, allocating inside the per-input loop would
			// churn 16 * 1MB = 16MB of Buffer.allocUnsafe per recovery block
			// (256MB total across all 16 recovery blocks) — wasteful since
			// mul_arr writes the whole `tmp` before we read it.
			var tmp = allocBuffer(blockSize);
			var coeff1 = Buffer.alloc(8);
			var len = Math.floor(blockSize / 8);

			// Cauchy sum:
			//   recoveryData = XOR over j of (inputBlocks[j] * coeffs[j])
			// mul_arr is element-wise: out[i] = in[i] * coeff[i % n_coeff].
			// With n_coeff=1, it acts as a scalar multiply. So we call it once
			// per input block (with that block's coefficient cycled across all
			// elements) and XOR the result into recoveryData in JS.
			for(var j = 0; j < numInput; j++) {
				coeff1.writeBigUInt64LE(coeffs[j], 0);
				this.encoder.mul_arr(tmp, inputBlocks[j], coeff1, len, 1);
				for(var i = 0; i < blockSize; i++) {
					recoveryData[i] ^= tmp[i];
				}
			}
