...
PAR3Gen.prototype = {
	...
	_processRecoveryBatch: function(batch, firstInputIdx, firstRecovery, numRecovery, accumulator) {
		...
		if(useJsKernel) {
			// === JS mul_arr path: process this batch's blocks into `accumulator` ===
			...
			// Build coefficient matrix for THIS batch only.
			//   coeff[recIdx][batchIdx] = 1/((firstInputIdx + batchIdx) + (firstRecovery + recIdx))
			...
			// via k % bufferCount because the XOR into accumulator happens
			...
			// Cache-batched Cauchy sum for this batch:
			//   accumulator[k * blockSize + i] ^= inputBlocks[batchIdx][i] * coeff[k][batchIdx]
			for(var batchIdx = 0; batchIdx < numBatch; batchIdx++) {
			...
			var partial = Buffer.alloc(numRecovery * blockSize);
			...
			// XOR partial results into the global accumulator.
			// By linearity of the Cauchy matrix multiplication, the sum of
			// partials across batches equals the single full computation.
			// Word-wise XOR via BigInt64Array (~8× faster than byte loop).
			var totalBytes = numRecovery * blockSize;
			var totalWords = totalBytes >> 3;
			var accView = new BigInt64Array(accumulator.buffer, accumulator.byteOffset, totalWords);
			var partView = new BigInt64Array(partial.buffer, partial.byteOffset, totalWords);
	...
	// `accumulator` is a Buffer of size numRecovery * blockSize.
	_finalizeRecoveryBlocks: function(accumulator, numInput, eventCb, cb) {
		...
		self._createRecoveryPackets(recoveryBlocks, function(err, packets) {
			...
			// Write packets with backpressure handling to avoid writev overflow
			var pktIdx = 0;
			function writeNext() {
				...
				if(!ok) {
					...
					// Use setImmediate to avoid stack overflow with many packets
...
run: function(eventCb, completeCb) {
		...
		function flushBatch() {
			...
			if(recoveryAccumulator === null) {
				recoveryAccumulator = Buffer.alloc(numRecovery * blockSize);
			}
			var batchStart = BigInt(processedBlocks - batchInputs.length);
			self._processRecoveryBatch(batchInputs, batchStart, firstRecovery, numRecovery, recoveryAccumulator);
		...
		var processNextFile = function(idx) {
			if(idx >= self.files.length) {
				...
				self._finalizeRecoveryBlocks(recoveryAccumulator, totalInputBlocks, eventCb, function(err) {
					...
					fs.close(fd, function() {
						...
						try {
							...
							if(binding.solve_and_reconstruct) {
								...
								var ok = binding.solve_and_reconstruct(A, rhsBlocks, n, blockSize);
								if(!ok) {
								...
								cb(new Error('solve_and_reconstruct not available in native module'));
							}
						} catch(e) {
