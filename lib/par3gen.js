...
// A3 (par3-1200mbps): module-level input buffer pool for the create path.
// Replaces per-block Buffer.alloc(toRead) in the read loop with a slice from
// a pre-allocated 64 MiB pool. The slice shares memory with the pool, so the
// underlying allocation happens once at module load — eliminating the GC
// pressure from the per-block allocations that previously dominated the
// create path at high block counts (e.g. 1 GiB at 4 KiB blocks = 262144
// per-block allocs).
//
// Size choice: 64 MiB > the largest typical block size of 4 MiB, so any
// single-block read fits with headroom. The offset is append-only within a
// create call and is reset between creates (and between files within a
// multi-file create) via _resetInputBufferPool(). For workloads whose total
// per-file read size exceeds 64 MiB, the pool throws a clear error — full
// LRU eviction is B2's job.
var INPUT_BUFFER_POOL_SIZE = 64 * 1024 * 1024;
var inputBufferPool = Buffer.alloc(INPUT_BUFFER_POOL_SIZE);
...
run: function(eventCb, completeCb) {
			...
			var processFileFull = function(idx) {
				...
				function readBlockFull() {
					...
					function readBatch() {
						...
						var afterChecksum = self._getDataAfterChecksum();
						items.forEach(function(item) {
	...
	// Same bytes as header.slice(24, 48) inside finalizePacketHeader —
	// needed bit-exact so the worker hash matches the inline path.
	_getDataAfterChecksum: function() {
		if(!this._dataAfterChecksum) {
			var ac = Buffer.alloc(24);
			writeUInt64LE(ac, PAR3_PKT_HDR_SIZE + 8 + this.opts.blockSize, 0);
			this.inputSetId.copy(ac, 8);
			ac.write(PAR3_PKT_TYPE.DATA, 16, 8, 'ascii');
			this._dataAfterChecksum = ac;
		}
		return this._dataAfterChecksum;
	},
