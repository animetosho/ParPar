...
module.exports = {
	...
	fileInfo: function(files, recurse, skipSymlinks, concurrency, cb) {
		...
		var bufs = new BufferPool([], 16384, concurrency); // use the BufferPool as concurrency control as well
			...
			async.eachSeries(files, function(file, cbNextFile) {
				bufs.get(function(buf) {
					...
					async.waterfall([
							...
							if(stat.isDirectory()) {
								...
								if(recurse) {
									fs.readdir(file, function(err, dirFiles) {
										bufs.put(buf); // release hold on I/O
										if(err) return cb(err);
						...
						if(err && err !== true) // 'err === true' is a hack for short wiring
							procErr = err;
						if(info) {
							results.push(info);
							bufs.put(buf);
				...
				// if there's no error, call doneScan when all files have been processed (see above)
				if(err && !doneCalled) {
					doneCalled = true;
					cbDoneScan(err);
				}
		...
		return results; // returns reference to incomplete list - caller can periodically check this to get the number of files scanned
	},
