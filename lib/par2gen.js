...
PAR2Gen.prototype = {
	...
	start: function(cb, cbProgress) {
		...
		// TODO: allow starting from an arbitrary pass (e.g. first pass done manually, then continue)
		async.timesSeries(this.passes, function(pass, cb) {
			...
			async.timesSeries(self.chunks, function(chunkPass, cb) {
				...
				async.series([
					// read & process data
					self.passNum == 0 // first pass needs to prepare output files as well
						? async.parallel.bind(async, [readFn, self._initOutputFiles.bind(self)])
						: readFn,
					function(cb) {
						// input data processed
						if(cbProgress) cbProgress('pass_complete', pass, chunkPass);
						self.finish(cb);
					},
					...
					function(cb) {
						self.passNum++;
						if(cbProgress) cbProgress('files_written', pass, chunkPass);
