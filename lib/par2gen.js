	...
	start: function(cb, cbProgress) {
		...
		async.series([
			...
			function(cb) {
				...
				if(self._chunker)
					self.chunkOffset = self._chunkSize;
				else
					self.sliceOffset = self._slicesPerPass;
				var buf = readBuf.slice(0, self._chunkSize);
				async.whilst(function(){return self.sliceOffset < o.recoverySlices;}, function(cb) {
					
					// set what slices are being generated
					if(self.sliceOffset + self._slicesPerPass > o.recoverySlices)
						self._slicesPerPass = o.recoverySlices - self.sliceOffset;
					
					if(!skipChunks) // if we're continuing from above, don't reset chunker
						self._setSlices(self.sliceOffset);
					
					
					async.timesSeries(self.chunks-skipChunks, function(n, cb) {
						var readSize = Math.min(self._chunkSize, o.sliceSize - self.chunkOffset);
						// resize if necessary
