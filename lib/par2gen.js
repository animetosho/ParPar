...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	if(o.inputParallelSubmit < 1 || o.inputParallelSubmit > 32768)
		throw new Error('Invalid input concurrency');
...
PAR2Gen.prototype = {
	...
	process: function(file, buf, cb) {
		if(this.passNum || this.passChunkNum) {
			if(this._chunker)
				this._chunker.process(file, buf, cb);
			else
				file.process(buf, cb);
		} else {
					...
					async.timesSeries(file.numSlices, function(sliceNum, cb) {
						...
						(function readLoop(cb) {
							...
							fs.read(fd, self._buf, 0, Math.min(sliceLeft, self.readSize), null, function(err, bytesRead) {
								...
								var tasks = [file.processHash.bind(file, self._buf.slice(0, bytesRead))];
								if(!chunkProcessed && self._chunker) { // first part - need to feed to chunker
									tasks.push(function(cb) {
										chunkProcessed = true;
										self._chunker.process(file, self._buf.slice(0, Math.min(chunkSize, bytesRead)), cb);
									});
								}
								async.parallel(tasks, readLoop.bind(null, cb));
							});
						})(cb);
					}, function(err) {
