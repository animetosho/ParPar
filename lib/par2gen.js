...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	if(passes > 1) {
		if(o.minChunkSize && o.memoryLimit && (this.procBufferOverheadCount+1) * o.minChunkSize > o.memoryLimit)
			throw new Error('Cannot accommodate requested memory limit (' + friendlySize(o.memoryLimit) + ') with specified minimum chunk size (' + friendlySize(o.minChunkSize) + '). At least ' + (this.procBufferOverheadCount+1) + ' buffers of the minimum chunk size need to be allocated.');
		...
		if(chunkSize < minChunkSize) {
			...
			this.slicesPerPass = Math.floor(o.memoryLimit / chunkSize) - this.procBufferOverheadCount;
			if(this.slicesPerPass < 1) throw new Error('Cannot accommodate specified memory limit');
