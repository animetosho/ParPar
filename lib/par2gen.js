...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	if(sliceSize < 0) {
		...
		if(-sliceSize < dataFiles) {
			...
			throw new Error('Cannot select slice size to satisfy required number of slices, as there are more non-empty files than the specified number of slices');
		}
		if(dataFiles == 0)
	...
	if(o.minChunkSize > MAX_BUFFER_SIZE_MOD2)
		throw new Error('Minimum chunk size exceeds maximum size supported by this version of Node.js of ' + MAX_BUFFER_SIZE_MOD2 + ' bytes');
	if(o.seqReadSize > MAX_BUFFER_SIZE_MOD2)
		throw new Error('Read buffer size exceeds maximum size supported by this version of Node.js of ' + MAX_BUFFER_SIZE_MOD2 + ' bytes');
	...
	if(o.readHashQueue > o.readBuffers)
		throw new Error('Read hash queue size cannot exceed number of read buffers');
	if(o.recDataSize) {
	...
	if(!o.processBatchSize) {
		...
		o.processBatchSize = Math.round(12 / multiple) * multiple;
	...
	var extraBufferCount = Math.max(o.processBatchSize*stagingCount, o.recDataSize);
	...
	if(o.minChunkSize) {
		...
		if(o.minChunkSize > o.seqReadSize)
			throw new Error('Minimum chunk size cannot exceed read buffer size');
	}
	...
	var reqMem = o.sliceSize * (o.recoverySlices + extraBufferCount);
	this.passes = 1;
	this.chunks = 1;
	var passes = o.memoryLimit ? Math.ceil(reqMem / o.memoryLimit) : 1;
	var minPasses = Math.ceil(o.sliceSize / maxChunkSize);
	passes = Math.max(passes, minPasses);
	if(passes > 1) {
		if(o.minChunkSize && o.memoryLimit && (extraBufferCount+1) * o.minChunkSize > o.memoryLimit)
			throw new Error('Cannot accommodate requested memory limit (' + friendlySize(o.memoryLimit) + ') with specified minimum chunk size (' + friendlySize(o.minChunkSize) + '). At least ' + (extraBufferCount+1) + ' buffers of the minimum chunk size need to be allocated.');
		...
		if(!o.memoryLimit && chunkSize < minChunkSize) {
			// chunkSize should be optimally < maxChunkSize in this case
			if(o.minChunkSize)
				minChunkSize = chunkSize; // ignore underflowing requested minimum
			else
				throw new Error('Cannot disable chunking as maximum chunk size is less than the slice size');
		}
		if(chunkSize < minChunkSize) {
			...
			var slicesPerPass = Math.floor(o.memoryLimit / chunkSize) - extraBufferCount;
			if(slicesPerPass < 1) throw new Error('Cannot accommodate specified memory limit');
	...
	var par = this.par2 = new Par2.PAR2(fileInfo, o.sliceSize, {method: gfInfo.id, threads: o.numThreads, batchSize: o.processBatchSize, stagingCount: stagingCount, hashBatchSize: o.hashBatchSize, tileSize: o.loopTileSize});
	...
	if(this.chunks > 1) {
		...
		if(this.readSize < this._chunkSize)
			throw new Error('Read size cannot be smaller than chunking size');
	}
		...
		if(o.memoryLimit && o.minChunkSize)
