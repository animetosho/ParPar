...
PAR2Gen.prototype = {
	...
	finish: function(cb) {
		...
		if(this._chunker)
			this._chunker.finish(this.files, cb);
		else
			this.par2.finish(this.files, cb);
	},
							...
							(function putData() {
								fs.read(fd, readBuf, 0, o.sliceSize, null, function(err, bytesRead) {
									...
									if(!bytesRead) { // EOF
										...
										self.process(file, readBuf.slice(0, bytesRead), putData);
									}
								});
							})();
						});
								...
								fs.open(file.name, 'r', function(err, fd) {
									...
									async.timesSeries(file.numSlices, function(sliceNum, cb) {
										fs.read(fd, _buf, 0, readSize, filePos, function(err, bytesRead) {
											...
											self.process(file, _buf.slice(0, bytesRead), cb);
										});
									}, function(err) {
										if(err) return cb(err);
										fs.close(fd, cb);
