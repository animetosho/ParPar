...
function calcNumRecoverySlices(unit, value, sliceSize, totalSize, files) {
	...
	if(unit == 'largest_files' || unit == 'smallest_files') {
		var sorted = files.sort(unit == 'largest_files' ? function(a, b) {
			return b.size-a.size;
		} : function(a, b) {
			return a.size-b.size;
		});
		var amt = Math.ceil(value);
		var selected = sorted.slice(0, amt).map(function(file) {
			return Math.ceil(file.size / sliceSize);
		});
		if(amt != value && amt <= selected.length) {
			// last file is only partially used
			selected[amt-1] *= value - Math.floor(value);
		}
		return selected.reduce(function(sum, blocks) {
			return sum + blocks;
		}, 0);
	}
	return value;
...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	var o = this.opts = {
		...
		sliceSizeMultiple: 4,
		recoverySlices: 0,
		recoverySlicesUnit: 'slices', // slices/count, ratio, bytes, largest_files or smallest_files
	...
	o.recoverySlices = calcNumRecoverySlices(o.recoverySlicesUnit, o.recoverySlices, o.sliceSize, this.totalSize, fileInfo);
	...
	if(o.minRecoverySlices !== null && o.minRecoverySlicesUnit)
		minRecSlices = Math.ceil(calcNumRecoverySlices(o.minRecoverySlicesUnit, o.minRecoverySlices, o.sliceSize, this.totalSize, fileInfo));
	if(o.maxRecoverySlices !== null && o.maxRecoverySlicesUnit)
		maxRecSlices = Math.floor(calcNumRecoverySlices(o.maxRecoverySlicesUnit, o.maxRecoverySlices, o.sliceSize, this.totalSize, fileInfo));
