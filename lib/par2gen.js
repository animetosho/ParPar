...
PAR2Gen.prototype = {
			...
			if(numSlices) {
				...
				for(var i=0; i<numSlices; i++) {
					...
					while(critWritten < critRatio*(i+1)) {
						...
						packets[pos++] = clone(pkt);
						critWritten++;
						critTotalSize += pkt.size;
					}
				}
				
				packets[pos] = clone(creator);
			} else {
				packets = critPackets.concat([clone(creator)]);
				critTotalSize = sumSize(critPackets);
			}
		} else {
			// no critical packet repetition - just dump a single copy at the end
			packets = Array(numSlices).concat(clone(critPackets), [clone(creator)]);
			for(var i=0; i<numSlices; i++) {
				packets[i] = {
					type: 'recovery',
					index: i + sliceOffset,
			...
			if(pkt.data) {
				(function(cbOpened) {
					...
					if(j > i+1) {
						...
						wPkt.forEach(function(pkt) {
							delete pkt.data;
							delete pkt.dataChunkOffset;
						});
					} else {
						fs.write(rf.fd, pkt.data, 0, pkt.data.length, rf.fPos == pos ? null : pos, cb);
						delete pkt.data;
					}
					rf.fPos = nextPos;
				});
	...
	runChunkPass: function(cbProgress, cb) {
		...
		var chunkSize = Math.min(this._chunkSize, this.opts.sliceSize - this.chunkOffset);
		// resize if necessary
		if(this._chunker && chunkSize != this._chunker.chunkSize)
