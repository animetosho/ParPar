...
PAR2Gen.prototype = {
	...
	_traverseRecoveryPacketRange: function(sliceOffset, numSlices, fn, cb) {
	...
	start: function(cb, cbProgress) {
		...
		var readBuf = new Buffer(o.sliceSize);
		...
		async.series([
			async.parallel.bind(async, [
				...
				function(cb) {
					...
					async.eachSeries(files, function(file, cb) {
						...
						fs.open(file.name, 'r', function(err, fd) {
							...
							(function putData() {
								fs.read(fd, readBuf, 0, o.sliceSize, null, function(err, bytesRead) {
									if(err) return cb(err);
									if(!bytesRead) { // EOF
										fs.close(fd, cb);
									} else {
										if(cbProgress) cbProgress('processing_slice', file, sliceNum++);
										file.process(readBuf.slice(0, bytesRead), function() {
											if(self._chunker)
												self._chunker.process(file, readBuf.slice(0, Math.min(bytesRead, self._chunkSize)), putData);
											else
												putData();
										});
									}
				...
				// open output files
				this._initOutputFiles.bind(this)
			]),
			function(cb) {
				// all recovery generated
				if(cbProgress) cbProgress('pass_complete', 0, 0);
				
				if(self._chunker)
					self._chunker.finish(files, cb);
				else
					par.finish(files, cb);
			},
			function(cb) {
				...
				async.eachSeries(self.recoveryFiles, function(rf, cb) {
					...
					async.eachSeries(rf.packets, function(pkt, cb) {
						if(pkt.type == 'recovery') {
							if(pktI < self._slicesPerPass) {
								if(self._chunker) {
									fs.write(rf.fd, self._chunker.recoveryData[pktI], 0, self._chunkSize, cPos + Par2.RECOVERY_HEADER_SIZE, cb);
								} else {
			...
			function(cb) {
				if(cbProgress) cbProgress('files_written');
				if(singlePass) return cb();
				
				var numChunks = Math.ceil(o.sliceSize / self._chunkSize);
				var skipChunks = self._chunker ? 1 : 0; // if we did the first pass as chunked, we can skip one chunk
				// deal with subsequent passes
				var sliceOffset = self._chunker ? 0 : self._slicesPerPass;
				var chunkOffset = self._chunker ? self._chunkSize : 0;
				var buf = readBuf.slice(0, self._chunkSize);
				async.whilst(function(){return sliceOffset < o.recoverySlices;}, function(cb) {
					
					// set what slices are being generated
					if(sliceOffset + self._slicesPerPass > o.recoverySlices)
						self._slicesPerPass = o.recoverySlices - sliceOffset;
					
					if(!skipChunks) // if we're continuing from above, don't reset chunker
						self._setSlices(sliceOffset);
					
					
					async.timesSeries(numChunks-skipChunks, function(n, cb) {
						var readSize = Math.min(self._chunkSize, o.sliceSize - chunkOffset);
						// resize if necessary
						...
						async.series([
								...
								fs.open(file.name, 'r', function(err, fd) {
									...
									async.timesSeries(file.numSlices, function(sliceNum, cb) {
										fs.read(fd, _buf, 0, readSize, filePos, function(err, bytesRead) {
											...
											if(self._chunker)
												self._chunker.process(file, _buf.slice(0, bytesRead), cb);
											else
							...
							self._chunker ? self._chunker.finish.bind(self._chunker, files) : par.finish.bind(par, files),
							function(cb) {
								self._traverseRecoveryPacketRange(sliceOffset, self._slicesPerPass, function(fd, idx, filePos, cb) {
									var pkt;
									if(self._chunker) {
										pkt = self._chunker.recoveryData[idx];
						...
						sliceOffset += self._slicesPerPass;
						chunkOffset = 0;
						skipChunks = 0;
						if(err || !self._chunker) return cb(err);
						
						// write chunk headers
						self._traverseRecoveryPacketRange(sliceOffset - self._slicesPerPass, self._slicesPerPass, function(fd, idx, filePos, cb) {
							var data = self._chunker.getHeader(idx);
