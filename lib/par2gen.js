...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	if(this.inputSlices > 32768) throw new Error('Too many input slices: ' + this.inputSlices + ' exceeds maximum of 32768. Please consider increasing the slice size, or reducing the amount of input data');
		...
		if(this._chunker)
			this._chunker.finish(this.files, function() {
				self._traverseRecoveryPacketRange(self.sliceOffset, self._slicesPerPass, function(pkt, idx) {
					pkt.setData(self._chunker.getRecoveryData(idx), self.chunkOffset + Par2.RECOVERY_HEADER_SIZE);
				});
				_cb();
			});
		else
