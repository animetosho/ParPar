...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	if(passes > 1) {
		...
		if(chunkSize < minChunkSize) {
			...
			if(this.slicesPerPass < 1) throw new Error('Cannot accommodate specified memory limit (' + friendlySize(o.memoryLimit) + '); a chunk size of ' + friendlySize(chunkSize) + ' was chosen, but a minimum of 1 recovery + ' + this.procBufferOverheadCount + ' processing chunks need to be held in memory');
...
PAR2Gen.prototype = {
	...
	_readPass: function(chunkSize, cbProgress, cb) {
		...
		if(seeking) {
			...
			FileChunkReader(this.files, this.opts.sliceSize, chunkSize, this.chunkOffset, bufPool, function(file, buffer, sliceNum, cb) {
				if(cbProgress) cbProgress('processing_slice', file, sliceNum);
				self._chunker.processData(file, buffer, cb);
			}, function(err) {
				if(err) cb(err);
				else bufPool.end(cb);
			});
