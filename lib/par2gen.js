...
PAR2Gen.prototype = {
	...
	// finish pass
	finish: function(cb, sliceOffset, chunkOffset) { // TODO: move sliceOffset to object var
		...
		if(this._chunker)
			this._chunker.finish(this.files, function() {
				self._traverseRecoveryPacketRange(sliceOffset || 0, self._slicesPerPass, function(pkt, idx) {
					pkt.data = self._chunker.recoveryData[idx];
					pkt.dataChunkOffset = chunkOffset + Par2.RECOVERY_HEADER_SIZE;
				});
				_cb();
			});
		else
			...
			if(pkt.data) {
				(function(cbOpened) {
					...
					if('dataChunkOffset' in pkt) {
						fs.write(rf.fd, pkt.data, 0, pkt.data.length, cPos + pkt.dataChunkOffset, cb);
						delete pkt.dataChunkOffset;
					} else {
						fs.write(rf.fd, pkt.data, 0, pkt.size, cPos, cb);
					}
					delete pkt.data;
				});
			} else
				...
				async.whilst(function(){return sliceOffset < o.recoverySlices;}, function(cb) {
					...
					async.timesSeries(numChunks-skipChunks, function(n, cb) {
						...
						async.series([
							...
							function(cb) {
								self.finish(cb, sliceOffset, chunkOffset);
							},
						...
						self._traverseRecoveryPacketRange(sliceOffset - self._slicesPerPass, self._slicesPerPass, function(pkt, idx) {
