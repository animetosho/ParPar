			...
			async.series([
				...
				function(cb) {
					...
					if(writev && pkt.dataChunkOffset + writeLen == pkt.size) {
						while(writeToPktI < rf.packets.length) {
							var nPkt = rf.packets[writeToPktI];
					...
					if(cbProgress) cbProgress('writing_file_pos', rf, pos);
					if(writev && writeLen <= MAX_WRITE_SIZE) {
						// can write combine
						var wPkt = rf.packets.slice(pktI, writeToPktI);
						var wBufs = Array.prototype.concat.apply([], wPkt.map(function(pkt) {
							return pkt.takeData();
						}));
						writev(rf.fd, wBufs, pos, cb);
					} else {
