...
var junkByte = (Buffer.from || Buffer)([255]);
...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	critPackets.push(new PAR2GenPacket('main', par.packetMainSize()));
	var creatorPkt = new PAR2GenPacket('creator', par.packetCreatorSize(o.creator));
		...
		async.eachSeries(files, function(file, cb) {
			...
			async.waterfall([
				...
				function(stat, cb) {
					...
					info.size = stat.size;
					if(!info.size) {
						info.md5 = info.md5_16k = (Buffer.from || Buffer)('d41d8cd98f00b204e9800998ecf8427e', 'hex'); // MD5 of blank string
