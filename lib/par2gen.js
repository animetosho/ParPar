...
function PAR2Gen(fileInfo, sliceSize, opts) {
	...
	if(this.readSize > 16*1048576) {
		// huge slices -> read in multiple calls
		// split into roughly 4MB chunks
		this.readSize = Math.ceil(o.sliceSize / Math.round(o.sliceSize / (4*1048576)))
	...
	} else if(this.readSize < 2*1048576) {
		// select a multiple of sliceSize which is >2MB
		this.readSize = Math.ceil(2*1048576 / o.sliceSize) * o.sliceSize;
	}
}
		...
		async.eachSeries(this.files, function(file, cb) {
			...
			fs.open(file.name, 'r', function(err, fd) {
				...
				if(seeking) {
					...
					if(!firstPass || !self._chunker) throw new Error('Cannot read less than 1 slice at a time');
					...
					async.timesSeries(file.numSlices, function(sliceNum, cb) {
						...
						(function readLoop(cb) {
							...
							fs.read(fd, self._buf, 0, Math.min(sliceLeft, self.readSize), null, function(err, bytesRead) {
								...
								if(chunkProcessed) { // first part - need to feed to chunker
									chunkProcessed = true;
