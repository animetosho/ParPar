...
function pickBestMethod() {
	...
	if(typeof binding.gf64_info !== 'function') {
		// macOS ARM64 fallback: native addon (parpar_gf64) doesn't compile,
		// so binding is missing the gf64_info export. Fall back to SCALAR,
		// which is the safe universal choice.
		process.stderr.write('Warning: native addon lacks gf64_info; using scalar fallback\n');
		return { method: 3, name: 'scalar', source: 'no-cpuid-info' };
	}
