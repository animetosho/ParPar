...
static HEDLEY_ALWAYS_INLINE void gf16_lookup_checksum_blocku(const void *HEDLEY_RESTRICT src, size_t amount, void *HEDLEY_RESTRICT checksum) {
	...
	if(sizeof(uintptr_t) >= 8) {
		...
		write64(checksum, (uint64_t)gf16_lookup_multi_mul2(read64(checksum)) ^ SWAP64(data));
	} else if(sizeof(uintptr_t) >= 4) {
		uint32_t data = 0;
		...
		write32(checksum, (uint32_t)gf16_lookup_multi_mul2(read32(checksum)) ^ SWAP32(data));
	} else {
		uint16_t data = 0;
		while(amount > 1) {
			data ^= read16(_src);
		...
		write16(checksum, (uint16_t)gf16_lookup_multi_mul2(read16(checksum)) ^ SWAP16(data));
	}
}
...
static HEDLEY_ALWAYS_INLINE void gf16_lookup_checksum_exp(void *HEDLEY_RESTRICT checksum, uint16_t exp) {
	uint16_t coeff = exp;
...
GF_FINISH_PACKED_FUNCS(gf16_lookup, _generic, gf16_lookup_stride(), gf16_lookup_copy_block, gf16_copy_blocku, 1, (void)0, gf16_lookup_checksum_block, gf16_lookup_checksum_blocku, gf16_lookup_checksum_exp, gf16_lookup_checksum_inline_finish, gf16_lookup_stride())
