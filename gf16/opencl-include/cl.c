...
int load_opencl() {
	...
	LOAD_FN(clRetainDevice)
	LOAD_FN(clReleaseDevice)
	LOAD_FN(clCreateContext)
	LOAD_FN(clCreateContextFromType)
	LOAD_FN(clRetainContext)
	LOAD_FN(clReleaseContext)
	LOAD_FN(clGetContextInfo)
	LOAD_FN(clCreateCommandQueueWithProperties)
	LOAD_FN(clRetainCommandQueue)
	LOAD_FN(clReleaseCommandQueue)
	LOAD_FN(clGetCommandQueueInfo)
	LOAD_FN(clCreateBuffer)
	LOAD_FN(clCreateSubBuffer)
	LOAD_FN(clCreateImage)
	LOAD_FN(clCreatePipe)
	LOAD_FN(clRetainMemObject)
	LOAD_FN(clReleaseMemObject)
	LOAD_FN(clGetSupportedImageFormats)
	LOAD_FN(clGetMemObjectInfo)
	LOAD_FN(clGetImageInfo)
	LOAD_FN(clGetPipeInfo)
	LOAD_FN(clSetMemObjectDestructorCallback)
	LOAD_FN(clSVMAlloc)
	LOAD_FN(clSVMFree)
	LOAD_FN(clCreateSamplerWithProperties)
	LOAD_FN(clRetainSampler)
	LOAD_FN(clReleaseSampler)
	LOAD_FN(clGetSamplerInfo)
	LOAD_FN(clCreateProgramWithSource)
	LOAD_FN(clCreateProgramWithBinary)
	LOAD_FN(clCreateProgramWithBuiltInKernels)
	LOAD_FN(clRetainProgram)
	LOAD_FN(clReleaseProgram)
	LOAD_FN(clBuildProgram)
	LOAD_FN(clCompileProgram)
	LOAD_FN(clLinkProgram)
	LOAD_FN(clUnloadPlatformCompiler)
	LOAD_FN(clGetProgramInfo)
	LOAD_FN(clGetProgramBuildInfo)
	LOAD_FN(clCreateKernel)
	LOAD_FN(clCreateKernelsInProgram)
	LOAD_FN(clRetainKernel)
	LOAD_FN(clReleaseKernel)
	LOAD_FN(clSetKernelArg)
	LOAD_FN(clSetKernelArgSVMPointer)
	LOAD_FN(clSetKernelExecInfo)
	LOAD_FN(clGetKernelInfo)
	LOAD_FN(clGetKernelArgInfo)
	LOAD_FN(clGetKernelWorkGroupInfo)
	LOAD_FN(clWaitForEvents)
	LOAD_FN(clGetEventInfo)
	LOAD_FN(clCreateUserEvent)
	LOAD_FN(clRetainEvent)
	LOAD_FN(clReleaseEvent)
	LOAD_FN(clSetUserEventStatus)
	LOAD_FN(clSetEventCallback)
	LOAD_FN(clGetEventProfilingInfo)
	LOAD_FN(clFlush)
	LOAD_FN(clFinish)
	LOAD_FN(clEnqueueReadBuffer)
	LOAD_FN(clEnqueueReadBufferRect)
	LOAD_FN(clEnqueueWriteBuffer)
	LOAD_FN(clEnqueueWriteBufferRect)
	LOAD_FN(clEnqueueFillBuffer)
	LOAD_FN(clEnqueueCopyBuffer)
	LOAD_FN(clEnqueueCopyBufferRect)
	LOAD_FN(clEnqueueReadImage)
	LOAD_FN(clEnqueueWriteImage)
	LOAD_FN(clEnqueueFillImage)
	LOAD_FN(clEnqueueCopyImage)
	LOAD_FN(clEnqueueCopyImageToBuffer)
	LOAD_FN(clEnqueueCopyBufferToImage)
	LOAD_FN(clEnqueueMapBuffer)
	LOAD_FN(clEnqueueMapImage)
	LOAD_FN(clEnqueueUnmapMemObject)
	LOAD_FN(clEnqueueMigrateMemObjects)
	LOAD_FN(clEnqueueNDRangeKernel)
	LOAD_FN(clEnqueueNativeKernel)
	LOAD_FN(clEnqueueMarkerWithWaitList)
	LOAD_FN(clEnqueueBarrierWithWaitList)
	LOAD_FN(clEnqueueSVMFree)
	LOAD_FN(clEnqueueSVMMemcpy)
	LOAD_FN(clEnqueueSVMMemFill)
	LOAD_FN(clEnqueueSVMMap)
	LOAD_FN(clEnqueueSVMUnmap)
	LOAD_FN(clGetExtensionFunctionAddressForPlatform)
//#ifdef CL_USE_DEPRECATED_OPENCL_1_1_APIS
	LOAD_FN(clCreateImage2D)
	LOAD_FN(clCreateImage3D)
	LOAD_FN(clEnqueueMarker)
	LOAD_FN(clEnqueueWaitForEvents)
	LOAD_FN(clEnqueueBarrier)
	LOAD_FN(clUnloadCompiler)
	LOAD_FN(clGetExtensionFunctionAddress)
//#endif
//#ifdef CL_USE_DEPRECATED_OPENCL_2_0_APIS
	LOAD_FN(clCreateCommandQueue)
	LOAD_FN(clCreateSampler)
	LOAD_FN(clEnqueueTask)
//#endif
