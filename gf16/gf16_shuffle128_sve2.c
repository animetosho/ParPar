...
static HEDLEY_ALWAYS_INLINE void gf16_shuffle128_sve2_round1(svuint8x2_t va, svuint8_t* rl, svuint8_t* rh,
	...
	svuint8_t tmp1 = NOMASK(svand_n_u8, svget2_u8(va, 0), 0xf);
	svuint8_t tmp2 = NOMASK(svlsr_n_u8, svget2_u8(va, 0), 4);
	*rl = NOMASK(sveor_u8, svtbl_u8(tbl_l0, tmp1), svtbl_u8(tbl_l1, tmp2));
	*rh = NOMASK(sveor_u8, svtbl_u8(tbl_h0, tmp1), svtbl_u8(tbl_h1, tmp2));
	
	tmp1 = NOMASK(svand_n_u8, svget2_u8(va, 1), 0xf);
	tmp2 = NOMASK(svlsr_n_u8, svget2_u8(va, 1), 4);
	*rl = sveor3_u8(*rl, svtbl_u8(tbl_l2, tmp1), svtbl_u8(tbl_l3, tmp2));
	*rh = sveor3_u8(*rh, svtbl_u8(tbl_h2, tmp1), svtbl_u8(tbl_h3, tmp2));
}
static HEDLEY_ALWAYS_INLINE void gf16_shuffle128_sve2_round(svuint8x2_t va, svuint8_t* rl, svuint8_t* rh,
	...
	svuint8_t tmp1 = NOMASK(svand_n_u8, svget2_u8(va, 0), 0xf);
	svuint8_t tmp2 = NOMASK(svlsr_n_u8, svget2_u8(va, 0), 4);
	*rl = sveor3_u8(*rl, svtbl_u8(tbl_l0, tmp1), svtbl_u8(tbl_l1, tmp2));
	*rh = sveor3_u8(*rh, svtbl_u8(tbl_h0, tmp1), svtbl_u8(tbl_h1, tmp2));
