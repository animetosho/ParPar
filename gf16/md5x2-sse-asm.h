...
#ifdef __XOP__
static HEDLEY_ALWAYS_INLINE void md5_process_block_x2_xop(__m128i* state, const char* const* HEDLEY_RESTRICT data, size_t offset) {
	UNUSED(offset);
	__m128i A = state[0];
	__m128i B = state[1];
	__m128i C = state[2];
...
#define ROUND_X(A, B, I, R) \
	"vpaddd %%xmm" STR(I) ", %[" STR(A) "], %[" STR(A) "]\n" \
	"vpaddd %%xmm4, %[" STR(A) "], %[" STR(A) "]\n" \
	"vprotd $" STR(R) ", %[" STR(A) "], %[" STR(A) "]\n" \
	"vpaddd %[" STR(B) "], %[" STR(A) "], %[" STR(A) "]\n"

#define ROUND_F(A, B, C, D, I, R) \
	"vpcmov %[" STR(B) "], %[" STR(D) "], %[" STR(C) "], %%xmm4\n" \
	ROUND_X(A, B, I, R)
#define ROUND_G(A, B, C, D, I, R) \
	"vpcmov %[" STR(D) "], %[" STR(C) "], %[" STR(B) "], %%xmm4\n" \
	ROUND_X(A, B, I, R)
#define ROUND_H(A, B, C, D, I, R) \
	"vpxor %[" STR(D) "], %[" STR(C) "], %%xmm4\n" \
	"vpxor %[" STR(B) "], %%xmm4, %%xmm4\n" \
...
#define RF4(offs, r1, r2) \
	...
	ROUND_F(A, B, C, D, 7,  7) \
	"vpsrlq $32, %%xmm7, %%xmm7\n" \
	ROUND_F(D, A, B, C, 7, 12) \
	ROUND_F(C, D, A, B, 6, 17) \
	"vpsrlq $32, %%xmm6, %%xmm6\n" \
	ROUND_F(B, C, D, A, 6, 22)
	
#ifdef PLATFORM_AMD64
#define BLENDD(r1, r2, target) \
	"vpblendw $0b11001100, " IN##r2 ", " IN##r1 ", %%" target "\n"
#else
#define BLENDD(r1, r2, target) \
	"vmovdqa " IN##r1 ", %%" target "\n" \
	"vpblendw $0b11001100, " IN##r2 ", %%" target ", %%" target "\n"
#endif
#define RG4(offs, rs, r1, r2) \
	BLENDD(r1, r2, "xmm5") \
	LOADK(offs) \
	...
	ROUND_G(D, A, B, C, 6,  9) \
	"vpsrlq $32, %%xmm6, %%xmm6\n" \
	ROUND_G(C, D, A, B, 6, 14) \
	ROUND_G(B, C, D, A, 7, 20)
...
#define RH4(offs, r1, r2, r3, r4) \
	...
	"vpsrlq $32, %%xmm7, %%xmm5\n" \
	ROUND_H(A, B, C, D, 5,  4) \
	ROUND_H(D, A, B, C, 7, 11) \
	"vpsrlq $32, %%xmm6, %%xmm5\n" \
	ROUND_H(C, D, A, B, 5, 16) \
	ROUND_H(B, C, D, A, 6, 23)
...
#define RI4(offs, r1, r2, r3, r4) \
	...
	ROUND_I(A, B, C, D, 7,  6) \
	"vpsrlq $32, %%xmm7, %%xmm7\n" \
	ROUND_I(D, A, B, C, 7, 10) \
	ROUND_I(C, D, A, B, 6, 15) \
	"vpsrlq $32, %%xmm6, %%xmm6\n" \
	ROUND_I(B, C, D, A, 6, 21)
