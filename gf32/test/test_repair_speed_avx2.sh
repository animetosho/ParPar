#!/bin/bash
# T9.10: Repair speed test with AVX2
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
PAR3_BIN="node $PROJECT_DIR/bin/par3.js"
TEST_DIR="/tmp/par3_repair_avx2_$$"
NUM_FILES=10
...
check_simd() {
    if ! grep -q 'avx2' /proc/cpuinfo 2>/dev/null; then
        log_warn "AVX2 not available - skipping test"
        exit 0
    fi
    log_info "AVX2 detected"
}
...
run_repair_speed_test() {
    log_info "Running repair with AVX2 method..."
    start_time=$(date +%s.%N)

    cd "$TEST_DIR"
    $PAR3_BIN repair -m avx2 -o output/test.par3 2>&1
    ...
    echo ""
    echo "=== AVX2 Repair Speed Results ==="
...
main() {
    echo "=========================================="
    echo " T9.10: Repair Speed (AVX2) Test"
    echo "=========================================="
    echo ""
    ...
    echo ""
    echo "=========================================="
    log_success "AVX2 repair speed test complete"
    echo "=========================================="
}
