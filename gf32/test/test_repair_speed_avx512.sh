#!/bin/bash
# T9.9: Repair speed test with AVX512
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
PAR3_BIN="node $PROJECT_DIR/bin/par3.js"
TEST_DIR="/tmp/par3_repair_avx512_$$"
NUM_FILES=10
...
check_simd() {
    if ! grep -q 'avx512' /proc/cpuinfo 2>/dev/null; then
        log_warn "AVX512 not available - skipping test"
        exit 0
    fi
    log_info "AVX512 detected"
}
...
run_repair_speed_test() {
    log_info "Running repair with AVX512 method..."
    start_time=$(date +%s.%N)

    cd "$TEST_DIR"
    $PAR3_BIN repair -m avx512 -o output/test.par3 2>&1
    ...
    echo ""
    echo "=== AVX512 Repair Speed Results ==="
...
main() {
    echo "=========================================="
    echo " T9.9: Repair Speed (AVX512) Test"
    echo "=========================================="
    echo ""
    ...
    echo ""
    echo "=========================================="
    log_success "AVX512 repair speed test complete"
    echo "=========================================="
}
